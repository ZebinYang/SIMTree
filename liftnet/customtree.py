...
class CustomMobTreeRegressor(MoBTreeRegressor, RegressorMixin):
    ...
    def __init__(self, base_estimator, param_dict={}, max_depth=2, min_samples_leaf=10, min_impurity_decrease=0, feature_names=None,
                 split_features=None, n_screen_grid=5, n_feature_search=5, n_split_grid=20, clip_predict=True, random_state=0, **kargs):
    ...
    def build_leaf(self, sample_indice):
        ...
        grid.fit(nx, self.y[sample_indice].ravel())
        best_estimator = grid.best_estimator_
        if self.clip_predict:
            predict_func = lambda x: best_estimator.predict(np.clip((x - mx) / sx, xmin, xmax))
        else:
            predict_func = lambda x: best_estimator.predict((x - mx) / sx)
